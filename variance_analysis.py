...
select_budget_forecast=0
current_forecast=0

Budget_2020_Raw, F1_2020_Raw, F2_2020_Raw, F3_2020_Raw = [pd.read_excel('C:/Users/Darragh/Documents/Python/Work/Data/Budget_2022.xlsx',sheet_name=x) 
...
def load_ledger_data():
    ...
    return pd.read_excel('C:/Users/Darragh/Documents/Python/Work/Data/NL_2022_05.xlsx')
...
with st.expander('Click to see Company YTD and Month PL'):
    ...
    with col1:
        ...
        if st.checkbox('Would you like to see the Forecast included in YTD above?'):
            forecast_selection = st.selectbox("Which Forecast do you want to see?",options = ["Forecast Q1", "Forecast Q2","Forecast Q3"],index=current_forecast,key='NL_YTD') # use index to default
    ...
    with col2:
        ...
        if st.checkbox('Would you like to see the Forecast included in above?'):
            forecast1_selection = st.selectbox("Which Forecast do you want to see included above?",options = ["Forecast Q1", "Forecast Q2","Forecast Q3"],index=current_forecast) # use index to default
            second_slot.dataframe (Budget_Actual.actual_v_forecast_month(forecast1_selection))


with st.expander('Would you like to see the Results for a specific Department?'):
    ...
    with col3:
        ...
        if st.checkbox('Would you like to see the Forecast included in Department results above?'):
            forecast_selection = st.selectbox("Which Forecast do you want to include?",options = ["Forecast Q1", "Forecast Q2","Forecast Q3"],index=current_forecast, key='Dept YTD') # use index to default
    ...
    with col4:    
        ...
        if st.checkbox('Would you like to see the Forecast included in Departmental Month Results?'):
            ...
            index=current_forecast, key='Dept Month') # use index to default https://discuss.streamlit.io/t/how-to-use-the-key-field-in-interactive-widgets-api/1007/11
            fourth_slot.dataframe (Budget_Actual.actual_v_forecast_month_dept(dept_selection,forecast2_selection))

with st.expander('Click for End of Year Projection'):
    ...
    if st.checkbox ('Select for Department'):
        dep_projection_sel=st.selectbox('Pick Department to run end of year projection', options = ["TV", "CG","Post","IT","Pipeline","Admin","Development","HR"],index=0)
...
#     with st.expander('Click to see the Gross Profit Variance for YTD v. Budget'):
...
# with col4:
with st.expander('Click to see the Gross Profit Variance for YTD v. Budget'):
    one_selection = st.selectbox("Which Budget/Forecast do you want to see?",options = ["Budget","Forecast Q1", "Forecast Q2","Forecast Q3"],index=select_budget_forecast,
    key='forecast_for_gp') # use index to default
    ...
    col_sales,col_cos = st.columns(2)
    with col_sales:
...
with st.expander('Click to see the Dept Gross Profit Variance for YTD v. Budget'):
    dep_sel=st.selectbox("Which Department do you want to see?",options = ['None',"TV", "CG",
    "Post","IT","Pipeline","Admin","Development","HR"],index=1, key='department selection for GP')
...
with st.expander('Actual & Budget/Forecast Revenue COS Breakdowns by Project'):
    column_one,column_second = st.columns(2)
    with column_one:
...
with st.expander('Gross Profit percent by Project'):
    ...
    st.write('revenue',long_format_function(dep_actual_revenue))
